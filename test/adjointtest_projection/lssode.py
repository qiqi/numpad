...
#                return self.t, self.u
            ...
            #tnew=self.t.copy()
            #tnew[1:] = tnew[0] + np.cumsum(G2)

            
            #projection2
            #deltax1 =(-1.0)*(v[:-1]*self.dudt).sum(1) + tnew[:-1] -self.t[:-1] 
            #deltax2 = (self.dudt*self.dudt).sum(1) +1

            #deltax = deltax1/deltax2
            #G1[:-1] = self.u[:-1] - self.dudt*deltax[:,np.newaxis]

            #projection
            #pr = (self.dudt * G1[1:]).sum(1) / (self.dudt * self.dudt).sum(1) 
            #G1[:-1] = G1[:-1] -  (self.dudt*pr[:,np.newaxis])
            #outputVector2d(G1,G1.shape, 'G1new'+str(counter)+'.dat')
